...
Queue* Queue_Create(size_t _maxSize)
{
    ...
    if((sem_init(&(queue->semEmpty), 0, _maxSize)) == -1)
	{
...
void Queue_Destroy(Queue** _queue, void (*_elementDestroy)(void* _item))
{
    ...
    sem_destroy(&(*_queue)->semFull);
    sem_destroy(&(*_queue)->semEmpty);
...
{       
    ...
    {
        printf("Error - overflow is invalid");
...
Queue_Result Queue_Dequeue(Queue* _queue, void** _pValue)
{
    ...
    {
        printf("Error - underflow is invalid");
